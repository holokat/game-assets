import { animateGranary, createGranary } from '../assets/granary.js';
import {
...
const EVIDENCE_PATH = 'review/granary/animation-evidence.json';
...
export async function exportGranary() {
  const authored = createGranary({ passId: 'optimization-pass' });
  ...
  animateGranary(authored, 1.25, 1);
  ...
  const animationEvidence = {
    ...
    changedPivots,
    chuteLever: {
      beforeRotationZ: before['chute-lever'][2],
      afterRotationZ: after['chute-lever'][2],
      deltaRadians: after['chute-lever'][2] - before['chute-lever'][2],
    },
    otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'chute-lever',
  };
  animateGranary(authored, 0, 1);
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Granary');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1,
      assetId: 'granary',
      generatedBy: 'src/export/exportGranary.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when chute-lever animation or gameplay semantics are required.',
        ],
      },
    };
...
window.__GRANARY_EXPORT_READY__ = false;
try {
  const manifest = await exportGranary();
  ...
  window.__GRANARY_EXPORT_MANIFEST__ = manifest;
  window.__GRANARY_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__GRANARY_EXPORT_ERROR__ = String(error.stack || error.message);
}
