import { createWaterTank } from '../assets/waterTank.js';
import {
...
export async function exportWaterTank() {
  const authored = createWaterTank({ passId: 'optimization-pass' });
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Water-Tank');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1,
      assetId: 'water-tank',
      generatedBy: 'src/export/exportWaterTank.js',
      ...
      optimized: {
        ...
        runtimeDecoderDependencies: [],
        optimizationNotes: [
          'All visible geometry is baked into world space and merged by compatible material signature.',
          'The optimized GLB is textureless and requires no mesh or texture decoder.',
          'Use the source GLB when valve articulation or gameplay semantics are required.',
        ],
      },
    };
...
window.__WATER_TANK_EXPORT_READY__ = false;
try {
  const manifest = await exportWaterTank();
  ...
  window.__WATER_TANK_EXPORT_MANIFEST__ = manifest;
  window.__WATER_TANK_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__WATER_TANK_EXPORT_ERROR__ = String(error.stack || error.message);
}
