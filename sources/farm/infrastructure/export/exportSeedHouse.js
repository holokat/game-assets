import { createSeedHouse } from '../assets/seedHouse.js';
import {
...
export async function exportSeedHouse() {
  const authored = createSeedHouse({ passId: 'optimization-pass' });
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Seed-House');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1,
      assetId: 'seed-house',
      generatedBy: 'src/export/exportSeedHouse.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when cleaner animation or gameplay semantics are required.',
        ],
      },
    };
...
window.__SEED_HOUSE_EXPORT_READY__ = false;
try {
  const manifest = await exportSeedHouse();
  ...
  window.__SEED_HOUSE_EXPORT_MANIFEST__ = manifest;
  window.__SEED_HOUSE_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__SEED_HOUSE_EXPORT_ERROR__ = String(error.stack || error.message);
}
