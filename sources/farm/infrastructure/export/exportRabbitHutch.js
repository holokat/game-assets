import { createRabbitHutch } from '../assets/rabbitHutch.js';
import {
...
export async function exportRabbitHutch() {
  ...
  const optimized = createStaticOptimizedRoot(authored, 'Rabbit-Hutch');
  try {
    ...
    const manifest = {
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB for the fold-down service hatch action and gameplay semantics.',
        ],
      },
    };
    await Promise.all([
...
window.__RABBIT_HUTCH_EXPORT_READY__ = false;
try {
  const manifest = await exportRabbitHutch();
  ...
  window.__RABBIT_HUTCH_EXPORT_MANIFEST__ = manifest;
  window.__RABBIT_HUTCH_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__RABBIT_HUTCH_EXPORT_ERROR__ = String(error.stack || error.message);
}
