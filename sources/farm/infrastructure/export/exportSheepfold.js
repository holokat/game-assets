import { createSheepfold } from '../assets/sheepfold.js';
...
export async function exportSheepfold() {
  const authored = createSheepfold();
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Sheepfold');
  try {
    const [sourceGlb, optimizedGlb] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1, assetId: 'sheepfold', coordinateSystem: { up: 'Y', forward: '+Z', unit: 'metre', groundY: 0 },
      source: {
        path: SOURCE_PATH, bytes: bytes(sourceGlb), ...collectSceneStats(source),
      ...
      optimized: {
        path: OPTIMIZED_PATH, bytes: bytes(optimizedGlb), ...collectSceneStats(optimized),
        semantics: { pivots: 0, sockets: 0, colliders: 0, destructionGroups: 0, animationChannels: 0 }, runtimeDecoderDependencies: [],
        optimizationNotes: ['Textureless visible geometry is baked and merged by material.', 'No mesh or texture decoder is required.', 'Use the source GLB for the broad entry gate action and gameplay semantics.'],
      },
    };
    ...
    disposeExportRoot(source); disposeExportRoot(optimized); authored.userData.sculptRuntime?.dispose();
  }
}
const output = document.querySelector('#output');
window.__SHEEPFOLD_EXPORT_READY__ = false;
try {
  const manifest = await exportSheepfold();
  ...
  window.__SHEEPFOLD_EXPORT_MANIFEST__ = manifest; window.__SHEEPFOLD_EXPORT_READY__ = true;
} catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__SHEEPFOLD_EXPORT_ERROR__ = String(error.stack || error.message); }
