import { animateGoosePen, createGoosePen } from '../assets/goosePen.js';
import {
...
const EVIDENCE_PATH = 'review/goose-pen/animation-evidence.json';
...
export async function exportGoosePen() {
  const authored = createGoosePen({ passId: 'optimization-pass' });
  ...
  animateGoosePen(authored, 1.25, 1);
  ...
  const animationEvidence = {
    assetId: 'goose-pen', method: 'Deterministic browser runtime state probe', timeSeconds: 1.25, intensity: 1,
    changedPivots,
    frontGate: {
      beforeRotationY: before['broad-front-gate'][1], afterRotationY: after['broad-front-gate'][1],
      deltaRadians: after['broad-front-gate'][1] - before['broad-front-gate'][1],
    },
    otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'broad-front-gate',
  };
  animateGoosePen(authored, 0, 1);
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Goose Pen');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1, assetId: 'goose-pen', generatedBy: 'src/export/exportGoosePen.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when front-gate animation or gameplay semantics are required.',
        ],
      },
    };
...
window.__GOOSE_PEN_EXPORT_READY__ = false;
try {
  const manifest = await exportGoosePen();
  ...
  window.__GOOSE_PEN_EXPORT_MANIFEST__ = manifest;
  window.__GOOSE_PEN_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__GOOSE_PEN_EXPORT_ERROR__ = String(error.stack || error.message);
}
