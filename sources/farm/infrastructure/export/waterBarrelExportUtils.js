...
export function collectRuntimeSemantics(root) {
  ...
  if (!runtime) throw new Error('Water Barrel must expose sculptRuntime before export.');
  return {
...
export function prepareWaterBarrelSource(root) {
  ...
  source.traverse((node) => { if (node.userData.collider) node.visible = true; if (node.isMesh) { node.castShadow = false; node.receiveShadow = false; } });
  source.updateMatrixWorld(true); return source;
}

export function createWaterBarrelStatic(root) {
  ...
  root.traverse((node) => {
    ...
    if (materials.length !== 1) throw new Error(`Multi-material mesh not supported: ${node.name}`);
    if (node.isInstancedMesh) {
      const instance = new THREE.Matrix4(); const world = new THREE.Matrix4();
      for (let index = 0; index < node.count; index += 1) { node.getMatrixAt(index, instance); world.multiplyMatrices(node.matrixWorld, instance); add(node, materials[0], world); }
    } else add(node, materials[0], node.matrixWorld);
  });
  const staticRoot = new THREE.Group(); staticRoot.name = 'Water-Barrel';
  ...
  for (const { material, geometries } of groups.values()) {
    ...
    const mesh = new THREE.Mesh(geometry, material.clone()); mesh.material.name = material.name;
    mesh.name = `water-barrel-static-${material.name.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}`; mesh.userData.staticMaterial = material.name; staticRoot.add(mesh);
  }
...
export async function exportBinaryGlb(root) {
  return new Promise((resolve, reject) => new GLTFExporter().parse(root, resolve, reject, { binary: true, onlyVisible: true, truncateDrawRange: true, maxTextureSize: 1024, includeCustomExtensions: false }));
}

export function disposeExportRoot(root) {
  const materials = new Set(); root.traverse((node) => { if (!node.isMesh) return; node.geometry?.dispose(); (Array.isArray(node.material) ? node.material : [node.material]).forEach((material) => materials.add(material)); }); materials.forEach((material) => material?.dispose());
}
