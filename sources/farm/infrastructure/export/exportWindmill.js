import { animateWindmill, createWindmill } from '../assets/windmill.js';
...
const MANIFEST_PATH = 'manifests/windmill.export.json'; const EVIDENCE_PATH = 'review/windmill/animation-evidence.json';
...
export async function exportWindmill() {
  const authored = createWindmill({ passId: 'optimization-pass' }); const runtime = authored.userData.sculptRuntime;
  const before = Object.fromEntries(Object.entries(runtime.pivots).map(([id, node]) => [id, node.rotation.toArray().slice(0, 3)])); animateWindmill(authored, 1.25, 1);
  ...
  const animationEvidence = { assetId: 'windmill', method: 'Deterministic browser runtime state probe', timeSeconds: 1.25, intensity: 1, changedPivots,
    sailHub: { beforeRotationZ: before['rotating-sail-hub'][2], afterRotationZ: after['rotating-sail-hub'][2], deltaRadians: after['rotating-sail-hub'][2] - before['rotating-sail-hub'][2] }, otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'rotating-sail-hub' };
  animateWindmill(authored, 0, 1); const semantics = collectRuntimeSemantics(authored); const source = prepareSourceHierarchy(authored); const optimized = createStaticOptimizedRoot(authored, 'Windmill');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = { schemaVersion: 1, assetId: 'windmill', generatedBy: 'src/export/exportWindmill.js', coordinateSystem: { up: 'Y', forward: '+Z', unit: 'metre', groundY: 0 },
...
const output = document.querySelector('#output'); window.__WINDMILL_EXPORT_READY__ = false;
try { const manifest = await exportWindmill(); output.textContent += `Source: ${manifest.source.bytes.toLocaleString()} bytes, ${manifest.source.drawCalls} draw calls, ${manifest.source.triangles.toLocaleString()} triangles.\n`; output.textContent += `Optimized: ${manifest.optimized.bytes.toLocaleString()} bytes, ${manifest.optimized.drawCalls} draw calls, ${manifest.optimized.triangles.toLocaleString()} triangles.\n`; window.__WINDMILL_EXPORT_MANIFEST__ = manifest; window.__WINDMILL_EXPORT_READY__ = true; }
catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__WINDMILL_EXPORT_ERROR__ = String(error.stack || error.message); }
