import { animateMillstone, createMillstone } from '../assets/millstone.js';
...
const SOURCE_PATH = 'source-glb/millstone.source.glb'; const OPTIMIZED_PATH = 'optimized-glb/Millstone.glb';
const MANIFEST_PATH = 'manifests/millstone.export.json'; const EVIDENCE_PATH = 'review/millstone/animation-evidence.json';
...
export async function exportMillstone() {
  const authored = createMillstone({ passId: 'optimization-pass' }); const runtime = authored.userData.sculptRuntime;
  const before = Object.fromEntries(Object.entries(runtime.pivots).map(([id, node]) => [id, node.rotation.toArray().slice(0, 3)])); animateMillstone(authored, 1.25, 1);
  ...
  const animationEvidence = { assetId: 'millstone', method: 'Deterministic browser runtime state probe', timeSeconds: 1.25, intensity: 1, changedPivots,
    upperMillstone: { beforeRotationY: before['rotating-upper-millstone'][1], afterRotationY: after['rotating-upper-millstone'][1], deltaRadians: after['rotating-upper-millstone'][1] - before['rotating-upper-millstone'][1] }, otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'rotating-upper-millstone' };
  animateMillstone(authored, 0, 1); const semantics = collectRuntimeSemantics(authored); const source = prepareSourceHierarchy(authored); const optimized = createStaticOptimizedRoot(authored, 'Millstone');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = { schemaVersion: 1, assetId: 'millstone', generatedBy: 'src/export/exportMillstone.js', coordinateSystem: { up: 'Y', forward: '+Z', unit: 'metre', groundY: 0 },
...
const output = document.querySelector('#output'); window.__MILLSTONE_EXPORT_READY__ = false;
try { const manifest = await exportMillstone(); output.textContent += `Source: ${manifest.source.bytes.toLocaleString()} bytes, ${manifest.source.drawCalls} draw calls, ${manifest.source.triangles.toLocaleString()} triangles.\n`; output.textContent += `Optimized: ${manifest.optimized.bytes.toLocaleString()} bytes, ${manifest.optimized.drawCalls} draw calls, ${manifest.optimized.triangles.toLocaleString()} triangles.\n`; window.__MILLSTONE_EXPORT_MANIFEST__ = manifest; window.__MILLSTONE_EXPORT_READY__ = true; }
catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__MILLSTONE_EXPORT_ERROR__ = String(error.stack || error.message); }
