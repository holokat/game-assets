import { createApiaryField } from '../assets/apiaryField.js';
import {
...
export async function exportApiaryField() {
  const authored = createApiaryField({ passId: 'optimization-pass' });
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Apiary-Field');
  try {
    ...
    const manifest = {
      ...
      generatedBy: 'src/export/exportApiaryField.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when hive, field, or socket semantics are required.',
        ],
      },
    };
...
window.__APIARY_FIELD_EXPORT_READY__ = false;
try {
  const manifest = await exportApiaryField();
  ...
  window.__APIARY_FIELD_EXPORT_MANIFEST__ = manifest;
  window.__APIARY_FIELD_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__APIARY_FIELD_EXPORT_ERROR__ = String(error.stack || error.message);
}
