import { createMeltwaterCollector } from '../assets/meltwaterCollector.js';
import {
...
export async function exportMeltwaterCollector() {
  const authored = createMeltwaterCollector({ passId: 'optimization-pass' });
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Meltwater-Collector');
  try {
    ...
    const manifest = {
      ...
      generatedBy: 'src/export/exportMeltwaterCollector.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when gameplay pivots, sockets, colliders, or destruction groups are required.',
        ],
      },
    };
...
window.__MELTWATER_COLLECTOR_EXPORT_READY__ = false;
try {
  const manifest = await exportMeltwaterCollector();
  ...
  window.__MELTWATER_COLLECTOR_EXPORT_MANIFEST__ = manifest;
  window.__MELTWATER_COLLECTOR_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__MELTWATER_COLLECTOR_EXPORT_ERROR__ = String(error.stack || error.message);
}
