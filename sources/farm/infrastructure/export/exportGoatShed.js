import { animateGoatShed, createGoatShed } from '../assets/goatShed.js';
import {
...
const EVIDENCE_PATH = 'review/goat-shed/animation-evidence.json';
...
export async function exportGoatShed() {
  const authored = createGoatShed({ passId: 'optimization-pass' });
  ...
  animateGoatShed(authored, 1.25, 1);
  ...
  const animationEvidence = {
    assetId: 'goat-shed', method: 'Deterministic browser runtime state probe', timeSeconds: 1.25, intensity: 1,
    changedPivots,
    serviceGate: {
      beforeRotationY: before['service-access-gate'][1], afterRotationY: after['service-access-gate'][1],
      deltaRadians: after['service-access-gate'][1] - before['service-access-gate'][1],
    },
    otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'service-access-gate',
  };
  animateGoatShed(authored, 0, 1);
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Goat Shed');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1, assetId: 'goat-shed', generatedBy: 'src/export/exportGoatShed.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when service-gate animation or gameplay semantics are required.',
        ],
      },
    };
...
window.__GOAT_SHED_EXPORT_READY__ = false;
try {
  const manifest = await exportGoatShed();
  ...
  window.__GOAT_SHED_EXPORT_MANIFEST__ = manifest;
  window.__GOAT_SHED_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__GOAT_SHED_EXPORT_ERROR__ = String(error.stack || error.message);
}
