import { animateAutomatedMilkingParlor, createAutomatedMilkingParlor } from '../assets/automatedMilkingParlor.js';
...
const MANIFEST_PATH = 'manifests/automated-milking-parlor.export.json'; const EVIDENCE_PATH = 'review/automated-milking-parlor/animation-evidence.json';
...
export async function exportAutomatedMilkingParlor() {
  const authored = createAutomatedMilkingParlor({ passId: 'optimization-pass' }); const runtime = authored.userData.sculptRuntime;
  const before = Object.fromEntries(Object.entries(runtime.pivots).map(([id, node]) => [id, node.rotation.toArray().slice(0, 3)])); animateAutomatedMilkingParlor(authored, 1.25, 1);
  ...
  const animationEvidence = { assetId: 'automated-milking-parlor', method: 'Deterministic browser runtime state probe', timeSeconds: 1.25, intensity: 1, changedPivots,
    entryGate: { beforeRotationY: before['entry-gate'][1], afterRotationY: after['entry-gate'][1], deltaRadians: after['entry-gate'][1] - before['entry-gate'][1] }, otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'entry-gate' };
  animateAutomatedMilkingParlor(authored, 0, 1); const semantics = collectRuntimeSemantics(authored); const source = prepareSourceHierarchy(authored); const optimized = createStaticOptimizedRoot(authored, 'Automated Milking Parlor');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = { schemaVersion: 1, assetId: 'automated-milking-parlor', generatedBy: 'src/export/exportAutomatedMilkingParlor.js', coordinateSystem: { up: 'Y', forward: '+Z', unit: 'metre', groundY: 0 },
...
const output = document.querySelector('#output'); window.__AUTOMATED_MILKING_PARLOR_EXPORT_READY__ = false;
try { const manifest = await exportAutomatedMilkingParlor(); output.textContent += `Source: ${manifest.source.bytes.toLocaleString()} bytes, ${manifest.source.drawCalls} draw calls, ${manifest.source.triangles.toLocaleString()} triangles.\n`; output.textContent += `Optimized: ${manifest.optimized.bytes.toLocaleString()} bytes, ${manifest.optimized.drawCalls} draw calls, ${manifest.optimized.triangles.toLocaleString()} triangles.\n`; window.__AUTOMATED_MILKING_PARLOR_EXPORT_MANIFEST__ = manifest; window.__AUTOMATED_MILKING_PARLOR_EXPORT_READY__ = true; }
catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__AUTOMATED_MILKING_PARLOR_EXPORT_ERROR__ = String(error.stack || error.message); }
