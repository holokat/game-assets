import { createAnimalPasture } from '../assets/animalPasture.js';
...
const SOURCE_PATH = 'source-glb/animal-pasture.source.glb'; const OPTIMIZED_PATH = 'optimized-glb/Animal-Pasture.glb'; const MANIFEST_PATH = 'manifests/animal-pasture.export.json'; const bytes = (v) => new Blob([v]).size;
...
export async function exportAnimalPasture() {
  const authored = createAnimalPasture(); const semantics = collectRuntimeSemantics(authored); const source = prepareSourceHierarchy(authored); const optimized = createStaticOptimizedRoot(authored, 'Animal-Pasture');
  try {
    const [sourceGlb, optimizedGlb] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = { schemaVersion: 1, assetId: 'animal-pasture', coordinateSystem: { up: 'Y', forward: '+Z', unit: 'metre', groundY: 0 },
      ...
      optimized: { path: OPTIMIZED_PATH, bytes: bytes(optimizedGlb), ...collectSceneStats(optimized), semantics: { pivots: 0, sockets: 0, colliders: 0, destructionGroups: 0, animationChannels: 0 }, runtimeDecoderDependencies: [], optimizationNotes: ['Textureless visible geometry is baked and merged by material.', 'No mesh or texture decoder is required.', 'Use the source GLB for gate action and gameplay semantics.'] } };
...
const output = document.querySelector('#output'); window.__ANIMAL_PASTURE_EXPORT_READY__ = false;
try { const manifest = await exportAnimalPasture(); output.textContent += `Source: ${manifest.source.bytes} bytes, ${manifest.source.triangles} triangles, ${manifest.source.drawCalls} draw calls.\nOptimized: ${manifest.optimized.bytes} bytes, ${manifest.optimized.triangles} triangles, ${manifest.optimized.drawCalls} draw calls.\n`; window.__ANIMAL_PASTURE_EXPORT_MANIFEST__ = manifest; window.__ANIMAL_PASTURE_EXPORT_READY__ = true; }
catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__ANIMAL_PASTURE_EXPORT_ERROR__ = String(error.stack || error.message); }
