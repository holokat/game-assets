import { animateFuelTank, createFuelTank } from '../assets/fuelTank.js';
import {
...
const EVIDENCE_PATH = 'review/fuel-tank/animation-evidence.json';
...
export async function exportFuelTank() {
  const authored = createFuelTank({ passId: 'optimization-pass' });
  ...
  animateFuelTank(authored, 1.25, 1);
  ...
  const animationEvidence = {
    assetId: 'fuel-tank', method: 'Deterministic browser runtime state probe', timeSeconds: 1.25, intensity: 1,
    changedPivots,
    valveHandwheel: {
      beforeRotationZ: before['valve-handwheel'][2],
      afterRotationZ: after['valve-handwheel'][2],
      deltaRadians: after['valve-handwheel'][2] - before['valve-handwheel'][2],
    },
    otherPivotsUnchanged: changedPivots.length === 1 && changedPivots[0] === 'valve-handwheel',
  };
  animateFuelTank(authored, 0, 1);
  const semantics = collectRuntimeSemantics(authored);
  const source = prepareSourceHierarchy(authored);
  const optimized = createStaticOptimizedRoot(authored, 'Fuel Tank');
  try {
    const [sourceBinary, optimizedBinary] = await Promise.all([exportBinaryGlb(source), exportBinaryGlb(optimized)]);
    const manifest = {
      schemaVersion: 1,
      assetId: 'fuel-tank',
      generatedBy: 'src/export/exportFuelTank.js',
      ...
      optimized: {
        ...
        optimizationNotes: [
          ...
          'Use the source GLB when valve-handwheel animation or gameplay semantics are required.',
        ],
      },
    };
...
window.__FUEL_TANK_EXPORT_READY__ = false;
try {
  const manifest = await exportFuelTank();
  ...
  window.__FUEL_TANK_EXPORT_MANIFEST__ = manifest;
  window.__FUEL_TANK_EXPORT_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__FUEL_TANK_EXPORT_ERROR__ = String(error.stack || error.message);
}
