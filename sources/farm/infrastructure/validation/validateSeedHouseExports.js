...
const SOURCE_PATH = './source-glb/seed-house.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Seed-House.glb';
const MANIFEST_PATH = './manifests/seed-house.export.json';
const RESULT_PATH = 'manifests/seed-house.validation.json';
...
export async function validateSeedHouseExports() {
  ...
  const metadata = source.getObjectByName('seed-house-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredSocketFamilies = ['terrain', 'service', 'loading', 'attachment', 'adjacency'];
  const socketFamilies = Object.fromEntries(requiredSocketFamilies.map((family) => [family, manifest.source.requiredSocketNames.filter((name) => name.includes(family))]));
  const checks = [
    ...
    check('only cleaner drum action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'seed-house-cleaner-drum', { actionChannels: manifest.source.actionChannels }),
...
window.__SEED_HOUSE_VALIDATION_READY__ = false;
try {
  const result = await validateSeedHouseExports();
  ...
  window.__SEED_HOUSE_VALIDATION_RESULT__ = result;
  window.__SEED_HOUSE_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__SEED_HOUSE_VALIDATION_ERROR__ = String(error.stack || error.message);
}
