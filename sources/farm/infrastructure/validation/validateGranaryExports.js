...
const SOURCE_PATH = './source-glb/granary.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Granary.glb';
const MANIFEST_PATH = './manifests/granary.export.json';
const RESULT_PATH = 'manifests/granary.validation.json';
...
export async function validateGranaryExports() {
  ...
  const metadata = source.getObjectByName('granary-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'loading', 'grain-output', 'service', 'attachment', 'adjacency'];
  ...
  const guardCount = manifest.source.requiredDestructionGroups['rodent-guards']?.length ?? 0;
  const checks = [
    ...
    check('six stone piers are authored', pierCount === 6, { pierCount }),
    check('six rodent guards are authored', guardCount === 6, { guardCount }),
    check('only chute lever action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'granary-chute-lever', { actionChannels: manifest.source.actionChannels }),
...
window.__GRANARY_VALIDATION_READY__ = false;
try {
  const result = await validateGranaryExports();
  ...
  window.__GRANARY_VALIDATION_RESULT__ = result;
  window.__GRANARY_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__GRANARY_VALIDATION_ERROR__ = String(error.stack || error.message);
}
