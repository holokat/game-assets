...
const RESULT_PATH = 'manifests/cistern.validation.json';
...
export async function validateCisternExports() {
  ...
  const metadata = source.getObjectByName('cistern-root')?.userData?.sourceSemantics;
  ...
  const checks = [
    ...
    check('source pivots survive reload', missing(nodeNames, manifest.source.requiredPivotNames).length === 0, { missing: missing(nodeNames, manifest.source.requiredPivotNames) }),
    check('source sockets survive reload', missing(socketNames, manifest.source.requiredSocketNames).length === 0, { missing: missing(socketNames, manifest.source.requiredSocketNames) }),
    check('source colliders survive reload', missing(colliderNames, manifest.source.requiredColliderNames).length === 0, { missing: missing(colliderNames, manifest.source.requiredColliderNames) }),
    check('source gameplay metadata survives reload', Boolean(metadata) && metadata.animationChannels?.length === 1, { animationChannels: metadata?.animationChannels?.length ?? 0 }),
    ...
    check('optimized material families survive reload', optimizedStats.materials.length >= 4, { materials: optimizedStats.materials }),
  ...
  const result = { schemaVersion: 1, assetId: 'cistern', result: checks.every((item) => item.passed) ? 'pass' : 'fail', checks, source: sourceStats, optimized: optimizedStats };
  await writeJson(RESULT_PATH, result);
  return result;
}

const output = document.querySelector('#output');
window.__CISTERN_VALIDATION_READY__ = false;
try {
  const result = await validateCisternExports();
  ...
  window.__CISTERN_VALIDATION_RESULT__ = result;
  window.__CISTERN_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__CISTERN_VALIDATION_ERROR__ = String(error.stack || error.message);
}
