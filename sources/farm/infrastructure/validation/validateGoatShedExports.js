...
const SOURCE_PATH = './source-glb/goat-shed.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Goat-Shed.glb';
const MANIFEST_PATH = './manifests/goat-shed.export.json';
const RESULT_PATH = 'manifests/goat-shed.validation.json';
...
const missing = (actual, expected) => { const present = new Set(actual); return expected.filter((value) => !present.has(value)); };
...
async function extensionsUsed(path) {
  ...
  const jsonLength = view.getUint32(12, true); const json = JSON.parse(new TextDecoder().decode(new Uint8Array(data, 20, jsonLength)).trim()); return json.extensionsUsed ?? [];
}
...
export async function validateGoatShedExports() {
  const [manifest, sourceGltf, optimizedGltf, sourceExtensions, optimizedExtensions] = await Promise.all([
    readJson(MANIFEST_PATH), new GLTFLoader().loadAsync(SOURCE_PATH), new GLTFLoader().loadAsync(OPTIMIZED_PATH), extensionsUsed(SOURCE_PATH), extensionsUsed(OPTIMIZED_PATH),
  ]);
  ...
  const nodeNames = namesWhere(source, () => true); const socketNames = namesWhere(source, (node) => Boolean(node.userData.socket));
  const colliderNames = namesWhere(source, (node) => Boolean(node.userData.collider));
  const metadata = source.getObjectByName('goat-shed-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'animal-entry', 'shelter', 'climbing-ramp', 'climbing-platform', 'feeding', 'watering', 'service', 'attachment', 'adjacency'];
  ...
  const pierCount = manifest.source.requiredDestructionGroups['stone-piers']?.length ?? 0;
  const checks = [
    ...
    check('source has exact ground contact', Math.abs(sourceStats.bounds.min[1]) <= GROUND_TOLERANCE, { minY: sourceStats.bounds.min[1], tolerance: GROUND_TOLERANCE }),
    check('optimized has exact ground contact', Math.abs(optimizedStats.bounds.min[1]) <= GROUND_TOLERANCE, { minY: optimizedStats.bounds.min[1], tolerance: GROUND_TOLERANCE }),
    ...
    check('source gameplay metadata survives GLTFLoader reload', Boolean(metadata) && metadata.animationChannels?.length === 1, { animationChannels: metadata?.animationChannels?.length ?? -1 }),
    check('required socket families are complete', Object.values(socketFamilies).every((values) => values.length > 0), { socketFamilies }),
    check('exactly six stone piers are authored', pierCount === 6, { pierCount }),
    check('ramp and side climbing platform are authored', ['goat-shed-ramp-deck-mesh', 'goat-shed-platform-deck-mesh'].every((name) => nodeNames.includes(name)), {}),
    check('only service access gate action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'goat-shed-service-access-gate', { actionChannels: manifest.source.actionChannels }),
    ...
    check('optimized uses 4 to 6 draw calls', optimizedStats.drawCalls >= 4 && optimizedStats.drawCalls <= 6, { drawCalls: optimizedStats.drawCalls, minimum: 4, maximum: 6 }),
  ...
  const result = { schemaVersion: 1, assetId: 'goat-shed', validationMethod: 'Browser GLTFLoader reload of source and optimized GLBs', result: checks.every((item) => item.passed) ? 'pass' : 'fail', checks, source: sourceStats, optimized: optimizedStats };
  await writeJson(RESULT_PATH, result); return result;
}

const output = document.querySelector('#output'); window.__GOAT_SHED_VALIDATION_READY__ = false;
try {
  const result = await validateGoatShedExports(); result.checks.forEach((item) => { output.textContent += `${item.passed ? 'PASS' : 'FAIL'}: ${item.name}\n`; });
  output.textContent += `Validation result: ${result.result.toUpperCase()}\n`; window.__GOAT_SHED_VALIDATION_RESULT__ = result; window.__GOAT_SHED_VALIDATION_READY__ = true;
} catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__GOAT_SHED_VALIDATION_ERROR__ = String(error.stack || error.message); }
