...
const SOURCE_PATH = './source-glb/meltwater-collector.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Meltwater-Collector.glb';
const MANIFEST_PATH = './manifests/meltwater-collector.export.json';
const RESULT_PATH = 'manifests/meltwater-collector.validation.json';
const GROUND_TOLERANCE = 0.006;
const MAX_TRIANGLES = 2500;
const MIN_DRAW_CALLS = 4;
...
export async function validateMeltwaterCollectorExports() {
  ...
  const metadata = source.getObjectByName('meltwater-collector-root')?.userData?.sourceSemantics;
  ...
  const checks = [
    ...
    check('optimized uses 4 to 6 draw calls', optimizedStats.drawCalls >= MIN_DRAW_CALLS && optimizedStats.drawCalls <= MAX_DRAW_CALLS, { drawCalls: optimizedStats.drawCalls, minimum: MIN_DRAW_CALLS, maximum: MAX_DRAW_CALLS }),
...
window.__MELTWATER_COLLECTOR_VALIDATION_READY__ = false;
try {
  const result = await validateMeltwaterCollectorExports();
  ...
  window.__MELTWATER_COLLECTOR_VALIDATION_RESULT__ = result;
  window.__MELTWATER_COLLECTOR_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__MELTWATER_COLLECTOR_VALIDATION_ERROR__ = String(error.stack || error.message);
}
