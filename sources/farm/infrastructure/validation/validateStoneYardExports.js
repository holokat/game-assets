...
const RESULT_PATH = 'manifests/stone-yard.validation.json';
...
export async function validateStoneYardExports() {
  ...
  const metadata = source.getObjectByName('stone-yard-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'loading', 'service', 'dressed-stone', 'stockpile', 'attachment', 'adjacency'];
  ...
  const pilePivots = nodeNames.filter((name) => /^stone-yard-aggregate-(coarse|medium|small)-pile$/.test(name));
  const blockStacks = [1, 2].map((index) => manifest.source.requiredDestructionGroups['dressed-blocks']?.filter((name) => name.includes(`stack-${index}-block-`)).length ?? 0);
  const checks = [
    ...
    check('exactly three aggregate piles are authored', pilePivots.length === 3, { pilePivots }),
    check('exactly two dressed-block stacks are authored', blockStacks.length === 2 && blockStacks.every((count) => count === 6), { blocksPerStack: blockStacks }),
    check('yard is intentionally static', manifest.source.actionChannels.length === 0, { actionChannels: manifest.source.actionChannels }),
  ...
  const result = {
    schemaVersion: 1, assetId: 'stone-yard', validationMethod: 'Browser GLTFLoader reload of source and optimized GLBs',
...
window.__STONE_YARD_VALIDATION_READY__ = false;
try {
  const result = await validateStoneYardExports();
  ...
  window.__STONE_YARD_VALIDATION_RESULT__ = result;
  window.__STONE_YARD_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__STONE_YARD_VALIDATION_ERROR__ = String(error.stack || error.message);
}
