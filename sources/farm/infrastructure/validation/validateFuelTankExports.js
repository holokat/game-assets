...
const SOURCE_PATH = './source-glb/fuel-tank.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Fuel-Tank.glb';
const MANIFEST_PATH = './manifests/fuel-tank.export.json';
const RESULT_PATH = 'manifests/fuel-tank.validation.json';
...
export async function validateFuelTankExports() {
  ...
  const metadata = source.getObjectByName('fuel-tank-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'fuel-input', 'fuel-output', 'service', 'level-gauge', 'spill-containment', 'attachment', 'adjacency'];
  ...
  const cradleBases = nodeNames.filter((name) => /^fuel-tank-cradle-[12]-base-mesh$/.test(name));
  const supportBands = nodeNames.filter((name) => /^fuel-tank-support-band-[12]-mesh$/.test(name));
  const checks = [
    ...
    check('exactly two cradle supports are authored', cradleBases.length === 2, { cradleBases }),
    check('exactly two support bands are authored', supportBands.length === 2, { supportBands }),
    check('only valve handwheel action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'fuel-tank-valve-handwheel', { actionChannels: manifest.source.actionChannels }),
  ...
  const result = {
    schemaVersion: 1, assetId: 'fuel-tank', validationMethod: 'Browser GLTFLoader reload of source and optimized GLBs',
...
window.__FUEL_TANK_VALIDATION_READY__ = false;
try {
  const result = await validateFuelTankExports();
  ...
  window.__FUEL_TANK_VALIDATION_RESULT__ = result;
  window.__FUEL_TANK_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__FUEL_TANK_VALIDATION_ERROR__ = String(error.stack || error.message);
}
