...
const SOURCE_PATH = './source-glb/water-tank.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Water-Tank.glb';
const MANIFEST_PATH = './manifests/water-tank.export.json';
const RESULT_PATH = 'manifests/water-tank.validation.json';
...
export async function validateWaterTankExports() {
  ...
  const metadata = source.getObjectByName('water-tank-root')?.userData?.sourceSemantics;
...
window.__WATER_TANK_VALIDATION_READY__ = false;
try {
  const result = await validateWaterTankExports();
  ...
  window.__WATER_TANK_VALIDATION_RESULT__ = result;
  window.__WATER_TANK_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__WATER_TANK_VALIDATION_ERROR__ = String(error.stack || error.message);
}
