...
const MANIFEST_PATH = './manifests/millstone.export.json'; const RESULT_PATH = 'manifests/millstone.validation.json';
...
export async function validateMillstoneExports() {
  ...
  const metadata = source.getObjectByName('millstone-root')?.userData?.sourceSemantics; const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  ...
  const grindingDiscs = ['millstone-lower-stone-disc-mesh', 'millstone-upper-stone-disc-mesh'].filter((name) => nodeNames.includes(name));
  const feedAndOutput = ['millstone-tapered-grain-hopper-mesh', 'millstone-flour-tray-floor-mesh'].filter((name) => nodeNames.includes(name));
  const checks = [
    ...
    check('two stacked grinding discs are authored', grindingDiscs.length === 2, { grindingDiscs }), check('grain hopper and flour tray are authored', feedAndOutput.length === 2, { feedAndOutput }),
    check('only upper millstone action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'millstone-rotating-upper-millstone', { actionChannels: manifest.source.actionChannels }),
  ...
  const result = { schemaVersion: 1, assetId: 'millstone', validationMethod: 'Browser GLTFLoader reload of source and optimized GLBs', result: checks.every((item) => item.passed) ? 'pass' : 'fail', checks, source: sourceStats, optimized: optimizedStats };
  await writeJson(RESULT_PATH, result); return result;
}

const output = document.querySelector('#output'); window.__MILLSTONE_VALIDATION_READY__ = false;
try { const result = await validateMillstoneExports(); result.checks.forEach((item) => { output.textContent += `${item.passed ? 'PASS' : 'FAIL'}: ${item.name}\n`; }); output.textContent += `Validation result: ${result.result.toUpperCase()}\n`; window.__MILLSTONE_VALIDATION_RESULT__ = result; window.__MILLSTONE_VALIDATION_READY__ = true; }
catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__MILLSTONE_VALIDATION_ERROR__ = String(error.stack || error.message); }
