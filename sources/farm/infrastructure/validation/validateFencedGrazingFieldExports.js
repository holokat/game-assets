...
const SOURCE_PATH = './source-glb/fenced-grazing-field.source.glb'; const OPTIMIZED_PATH = './optimized-glb/Fenced-Grazing-Field.glb';
const MANIFEST_PATH = './manifests/fenced-grazing-field.export.json'; const RESULT_PATH = 'manifests/fenced-grazing-field.validation.json';
...
export async function validateFencedGrazingFieldExports() {
  ...
  const metadata = source.getObjectByName('fenced-grazing-field-root')?.userData?.sourceSemantics; const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'grazing', 'feeding', 'watering', 'service', 'attachment', 'adjacency'];
  ...
  const cornerCount = manifest.source.requiredDestructionGroups['corner-posts']?.length ?? 0; const midpointCount = manifest.source.requiredDestructionGroups['midpoint-posts']?.length ?? 0;
  const fixedRailCount = manifest.source.requiredDestructionGroups['fence-rails']?.length ?? 0;
  const checks = [
    ...
    check('four corner and four midpoint posts are authored', cornerCount === 4 && midpointCount === 4, { cornerCount, midpointCount }),
    check('fixed fence sections use three rails each', fixedRailCount === 24, { fixedRailCount }),
    check('only broad front gate action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'fenced-grazing-field-broad-front-gate', { actionChannels: manifest.source.actionChannels }),
  ...
  const result = { schemaVersion: 1, assetId: 'fenced-grazing-field', validationMethod: 'Browser GLTFLoader reload of source and optimized GLBs', result: checks.every((item) => item.passed) ? 'pass' : 'fail', checks, source: sourceStats, optimized: optimizedStats };
  await writeJson(RESULT_PATH, result); return result;
}

const output = document.querySelector('#output'); window.__FENCED_GRAZING_FIELD_VALIDATION_READY__ = false;
try { const result = await validateFencedGrazingFieldExports(); result.checks.forEach((item) => { output.textContent += `${item.passed ? 'PASS' : 'FAIL'}: ${item.name}\n`; }); output.textContent += `Validation result: ${result.result.toUpperCase()}\n`; window.__FENCED_GRAZING_FIELD_VALIDATION_RESULT__ = result; window.__FENCED_GRAZING_FIELD_VALIDATION_READY__ = true; }
catch (error) { output.textContent += `${error.stack || error.message}\n`; window.__FENCED_GRAZING_FIELD_VALIDATION_ERROR__ = String(error.stack || error.message); }
