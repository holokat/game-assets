...
const SOURCE_PATH = './source-glb/goose-pen.source.glb';
const OPTIMIZED_PATH = './optimized-glb/Goose-Pen.glb';
const MANIFEST_PATH = './manifests/goose-pen.export.json';
const RESULT_PATH = 'manifests/goose-pen.validation.json';
...
export async function validateGoosePenExports() {
  const [manifest, sourceGltf, optimizedGltf, sourceExtensions, optimizedExtensions] = await Promise.all([
    readJson(MANIFEST_PATH), new GLTFLoader().loadAsync(SOURCE_PATH), new GLTFLoader().loadAsync(OPTIMIZED_PATH),
    extensionsUsed(SOURCE_PATH), extensionsUsed(OPTIMIZED_PATH),
  ]);
  ...
  const metadata = source.getObjectByName('goose-pen-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'animal-entry', 'loading', 'feeding', 'watering', 'shelter', 'service', 'attachment', 'adjacency'];
  ...
  const fenceRails = nodeNames.filter((name) => /^goose-pen-(front-left|front-right|rear|left|right)-rail-[123]-mesh$/.test(name));
  const checks = [
    ...
    check('tall fence uses three rails on every panel', fenceRails.length === 15, { railCount: fenceRails.length }),
    check('open corner shelter is authored', ['goose-pen-shelter-back-wall-mesh', 'goose-pen-shelter-side-wall-mesh', 'goose-pen-shelter-canopy-mesh'].every((name) => nodeNames.includes(name)), {}),
    check('only broad front gate action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'goose-pen-broad-front-gate', { actionChannels: manifest.source.actionChannels }),
    ...
    check('optimized uses 4 to 7 draw calls', optimizedStats.drawCalls >= 4 && optimizedStats.drawCalls <= 7, { drawCalls: optimizedStats.drawCalls, minimum: 4, maximum: 7 }),
  ...
  const result = {
    schemaVersion: 1, assetId: 'goose-pen', validationMethod: 'Browser GLTFLoader reload of source and optimized GLBs',
    result: checks.every((item) => item.passed) ? 'pass' : 'fail', checks, source: sourceStats, optimized: optimizedStats,
  };
...
window.__GOOSE_PEN_VALIDATION_READY__ = false;
try {
  const result = await validateGoosePenExports();
  ...
  window.__GOOSE_PEN_VALIDATION_RESULT__ = result;
  window.__GOOSE_PEN_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__GOOSE_PEN_VALIDATION_ERROR__ = String(error.stack || error.message);
}
