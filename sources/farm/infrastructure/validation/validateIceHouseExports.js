...
const RESULT_PATH = 'manifests/ice-house.validation.json';
...
export async function validateIceHouseExports() {
  ...
  const metadata = source.getObjectByName('ice-house-root')?.userData?.sourceSemantics;
  const boundsDelta = maxBoundsDelta(sourceStats, optimizedStats);
  const requiredFamilies = ['ground', 'terrain', 'loading', 'storage', 'cooling', 'ventilation', 'service', 'attachment', 'adjacency'];
  ...
  const roofVentStacks = nodeNames.filter((name) => /^ice-house-roof-vent-[12]-stack-mesh$/.test(name));
  const checks = [
    ...
    check('exactly two roof vents are authored', roofVentStacks.length === 2, { roofVentStacks }),
    check('only heavy front door action channel is authored', manifest.source.actionChannels.length === 1 && manifest.source.actionChannels[0].node === 'ice-house-heavy-front-door', { actionChannels: manifest.source.actionChannels }),
    ...
    check('source preserves building, chute, and door colliders', ['ice-house-collider-building', 'ice-house-collider-loading-chute', 'ice-house-collider-door'].every((name) => colliderNames.includes(name)), { colliderNames }),
  ];
  const result = {
    schemaVersion: 1,
    assetId: 'ice-house',
...
window.__ICE_HOUSE_VALIDATION_READY__ = false;
try {
  const result = await validateIceHouseExports();
  ...
  window.__ICE_HOUSE_VALIDATION_RESULT__ = result;
  window.__ICE_HOUSE_VALIDATION_READY__ = true;
} catch (error) {
  output.textContent += `${error.stack || error.message}\n`;
  window.__ICE_HOUSE_VALIDATION_ERROR__ = String(error.stack || error.message);
}
